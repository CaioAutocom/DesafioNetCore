...
public class CreatePersonValidator : AbstractValidator<CreatePersonRequest>
{
    ...
    public CreatePersonValidator(IUnitOfWork unitOfWork)
    {
        ...
        RuleFor(x => x.Document).MinimumLength(11).WithMessage("The document must contain at least 11 characteres.");
        RuleFor(x => x.Document).MaximumLength(14).WithMessage("The document must contain max 14 characteres.");
